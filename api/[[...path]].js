...
async function buildHealthResponse() {
  ...
  const allCriticalOk = mongodb.connected && checks.auth.jwtConfigured && checks.auth.adminLoginConfigured;
  return {
...
function createApiRouter() {
  ...
  api.get("/health/ready", async (c) => {
    ...
    return c.json(ok({ ready, timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
  });
